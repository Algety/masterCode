...
In this adaptation of the game, a pin code is randomly generated by the computer using 8 colours. A player can choose a difficulty level - easy (code of 4 pins) and medium (code of 5 pins). The colours of the pins in the code can duplicate. A player drags pins of 8 colours from the palette box to the answer box on the left. After the submit button is clicked, feedback for the attempt (red and yellow pins) is given in the feedback (clue) box on the right. Red pins indicate that a player has guessed a pin of the correct colour and in the correct position, while yellow pins indicate a pin of the correct colour but in the wrong position. The position of the feedback pins doesn’t indicate which of the pins in the answer box are guessed correctly.
...
## Testing
### Functional testing
* The game page is displayed correctly across all screen sizes and devices.
...
During the testing process at various stages of development, several bugs were identified and resolved, such as incorrect checking of player answers, displaying the wrong number of pins in the clue box, and duplication of dragged elements in the drop zone. These issues have been fixed before passing the game for the user tests.
